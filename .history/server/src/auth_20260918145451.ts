import { randomBytes, scryptSync } from 'node:crypto'; // nodeJS
...
export async function register(
    ...
    const [exists] = await pool.query('SELECT id FROM users WHERE username = ?', [username]); // nodeJS
...
// 3. 对比 1 和 2，成功则生成token randomBytes(32).toString('hex')并insert库
