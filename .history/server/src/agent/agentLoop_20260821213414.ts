        ...
        for (const call of step.toolCalls) {
            ...
            if (!tool) {
                ...
                if (tool.requiresApproval) {
                    // ★ 闸门：危险工具，先过人工这一关
                    const approval = createApproval(tool.name, args);   // 1. 建审批单（pending）
                    onEvent?.({
                        type: 'approval_required',
                    ...
                    const decision = await waitForApproval(approval.id); // 3. ★挂起：没人批之前停在这里
                    result = decision.status === 'approved'
                        ? tool.execute(args)                            // 4a. 批了 → 才真正执行
                        : { rejected: true, message: `操作被拒绝: ${tool.name}` }; // 4b. 拒了 → 不执行
                } else {
                    result = tool.execute(args);                        // 安全工具，直接执行
                }
            }
            // 广播：content
            onEvent?.({
