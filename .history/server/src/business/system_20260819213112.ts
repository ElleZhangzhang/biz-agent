...
export function updateOrderStatus(
    ...
    // 1. 找订单，找不到 → { ok: false, error: '订单不存在' }
    const order = orders.find(o => o.id === orderId);
    if (!order) return { ok: false, error: '订单不存在' }
    // 2. 校验：NEXT_STATUS[order.status] 里有没有 nextStatus？没有 → { ok: false, error: '不允许从 X 流转到 Y' }
    if (NEXT_STATUS[order.status]) return { ok: false, error: '不允许从 X 流转到 Y' }
