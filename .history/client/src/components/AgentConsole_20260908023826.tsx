        ...
        try {
            await runAgentStream(prompt, (event) => {
                if (event.type === 'answer_delta') {
                    // 增量文字：追加到正在打字的回答气泡（没有就新建一个）
                    setMessages((prev) => {
                        const last = prev[prev.length - 1];
                        if (last?.kind === 'answer_stream') {
                            return [...prev.slice(0, -1), { kind: 'answer_stream', text: last.text + event.content }];
                        }
                        return [...prev, { kind: 'answer_stream', text: event.content }];
                    });
                } else if (event.type === 'done') {
                    // done = 打字结束：用完整答案替换打字中的气泡，没有打字气泡就直接新增
                    // 兜底意义：即使中途丢了几个 delta，最终文本也不会缺字
                    setMessages((prev) => {
                        const last = prev[prev.length - 1];
                        if (last?.kind === 'answer_stream') {
                            return [...prev.slice(0, -1), { kind: 'answer', text: event.content }];
                        }
                        return [...prev, { kind: 'answer', text: event.content }];
                    });
                } else {
